...
export default class Application {
  ...
  _smoothTickRate(dt) {
    ...
    if (t.currentHz < target) {
      ...
      t.currentHz = Math.min(target, t.currentHz + Tick.RAMP_UP_RATE * target * dt);
    } else if (t.currentHz > target) {
      // ── Rule 7: decrease gradually ────────────────────────────────────
      t.currentHz = Math.max(target, t.currentHz - Tick.RAMP_DOWN_RATE * target * dt);
    }
  ...
  _tick_loop() {
    ...
    if (gameStarted && !frozen) {
      this.world._updateAmbients(now, rawDt, this.world.car.x, this.world.car.z);
    }
    ...
    if (this.input.enter && gameStarted && GameState.mode === 'ROAM') {
      if (!this._enterCooldown) {
        ...
        setTimeout(() => { this._enterCooldown = false; }, 800);
      }
    }
    ...
    if (gameStarted && (f % 3 === 0)) {
      ...
      this.audio.updateSpatialListener(
        this.world.car.x, this.world.car.z,
        this.world.car.sinA, this.world.car.cosA
      );
    }
    ...
    if (gameStarted && (f % 4 === 0)) {
      ...
      window.CityUI?.updateMinimap?.(
        this.world.car.x, this.world.car.z, -this.world.car.angle
      );
    }
  ...
  _exposePublicAPI() {
    ...
    window.CityEngine = {
      ...
      initAudio()   { self.audio.init(); },
      resetCamera() {
        GameState.mode = 'ROAM';
        GameState.focusedTempleId = null;
        self.camera.returnToFollow();
      },
      enterNearestBuilding()  { self._enterNearestTemple(); },
      cycleWeather() {
      ...
      setMusicVolume(v)     { self.audio.setMusicVolume(v); },
      skipGuide()           {},
      closeWorldPanel()     {},
      toggleYatraPath()     {},
      checkCompletion()     {},
      ...
      get narrativePhase()     { return 'FREE'; },
      get isNight()            { return self.world.isNight; },
      get currentWeather()     { return self._lastWeather || 'day'; },
      get weatherGrip()        { return self.world.car._weatherGrip; },
      get proximityBuilding()  {
        return window.CITY_DATA?.buildings.find(
          b => b.id === GameState.proximityTempleId) || null;
      },
      get carSpeed()  { return self.world.car.speed; },
      get carX()      { return self.world.car.x; },
      get carZ()      { return self.world.car.z; },
      get carAngle()  { return self.world.car.angle; },
      get camState()  { return self.camera.state; },

      // Debug info for devtools
      get tickHz()    { return Math.round(self._tick.currentHz); },
      get subSteps()  { return self._tick.subSteps; },
    };
  }
  ...
  _enterNearestTemple() {
    ...
    this.camera.focusOn(building, this.world.car.x, this.world.car.z, this.world.car.angle);
    setTimeout(() => window.CityUI?.openBuilding?.(building), 1400);
  }

  _triggerBloom(glowColor) {
    let bloom = document.getElementById('city-bloom');
    if (!bloom) {
      bloom = document.createElement('div');
      bloom.id = 'city-bloom';
      bloom.style.cssText = 'position:fixed;inset:0;z-index:9999;pointer-events:none;transition:background 0.2s ease';
      document.body.appendChild(bloom);
    }
    const hex = (glowColor || '#ffcc44').replace('#','');
    const r = parseInt(hex.slice(0,2),16), g = parseInt(hex.slice(2,4),16), b = parseInt(hex.slice(4,6),16);
    bloom.style.background = `rgba(${r},${g},${b},0.35)`;
    setTimeout(() => { bloom.style.background = 'rgba(0,0,0,0)'; }, 280);
